#!/usr/bin/env python3
"""
Backtest Hedge + DCA + BOOST MODE Strategy
==========================================
Tests the "Boost Mode" strategy during prolonged one-directional moves.

BOOST MODE RULES:
- When one side hits DCA 2, the OTHER side enters BOOST mode
- Boosted side: Double entry, TP closes half, keeps half, re-enters normal
- Boosted side still follows normal DCA if price reverses
- Exit boost mode when the DCA side recovers (TP or SL)
"""
...
class BoostModeBacktester:
    def __init__(self, symbol: str, start_balance: float = 100.0):
        ...
        # Positions - now support multiple positions per side for boost mode
        ...
        self.held_positions = []  # Positions being held (half kept after TP)
    ...
    def open_position(self, side: str, price: float, double_size: bool = False) -> dict:
        """Open new position"""
        budget = self.start_balance * self.budget_split
        initial_margin = budget * self.dca_pcts[0]

        # Double size if in boost mode
        if double_size:
            initial_margin *= 2
        ...
        position = {
            ...
            "is_boosted": double_size
        }
        return position

    def close_position(self, position: dict, exit_price: float, exit_type: str, timestamp, partial: float = 1.0):
        ...
        # PNL is based on margin used (partial = fraction being closed)
        margin_used = position["margin"] * partial
        pnl = margin_used * roi
        ...
        self.trades.append({
            ...
            "quantity": position["quantity"] * partial,
    ...
    def activate_boost_mode(self, trigger_side: str, timestamp):
        """Activate boost mode when one side hits DCA 2"""
        ...
        print(f"[{timestamp}] >>> BOOST MODE ACTIVATED! {trigger_side} at DCA 2 -> {self.boosted_side} BOOSTED")
    ...
    def deactivate_boost_mode(self, timestamp, reason: str):
        ...
        for held in self.held_positions:
            # These will be closed at current price in the main loop
            pass
    ...
    def run_backtest(self, df: pd.DataFrame):
        """Run the backtest with boost mode"""
        print("\n" + "="*70)
        print("RUNNING HEDGE + DCA + BOOST MODE BACKTEST")
        print("="*70)
        print(f"Symbol: {self.symbol}")
        print(f"Starting Balance: ${self.start_balance:.2f}")
        print(f"Leverage: {self.leverage}x")
        print(f"TP ROI: {self.tp_roi*100:.0f}% | SL ROI: {self.sl_roi*100:.0f}%")
        print(f"BOOST MODE: Activates at DCA 2, doubles opposite side")
        print("="*70)
        ...
        for i, (timestamp, row) in enumerate(df.iterrows()):
            ...
            # Check held positions first (from boost mode)
            for held in self.held_positions[:]:  # Copy list to modify during iteration
                if held["side"] == "LONG":
                    # Check TP
                    if high >= held["tp_price"]:
                        pnl = self.close_position(held, held["tp_price"], "TP_HELD", timestamp)
                        self.held_position_profits += pnl
                        print(f"[{timestamp}] HELD LONG TP @ ${held['tp_price']:.4f} | P&L: ${pnl:+.2f}")
                        self.held_positions.remove(held)
                    # Check SL
                    elif low <= held["sl_price"]:
                        pnl = self.close_position(held, held["sl_price"], "SL_HELD", timestamp)
                        self.held_position_profits += pnl
                        print(f"[{timestamp}] HELD LONG SL @ ${held['sl_price']:.4f} | P&L: ${pnl:+.2f}")
                        self.held_positions.remove(held)
                else:  # SHORT
                    # Check TP
                    if low <= held["tp_price"]:
                        pnl = self.close_position(held, held["tp_price"], "TP_HELD", timestamp)
                        self.held_position_profits += pnl
                        print(f"[{timestamp}] HELD SHORT TP @ ${held['tp_price']:.4f} | P&L: ${pnl:+.2f}")
                        self.held_positions.remove(held)
                    # Check SL
                    elif high >= held["sl_price"]:
                        pnl = self.close_position(held, held["sl_price"], "SL_HELD", timestamp)
                        self.held_position_profits += pnl
                        print(f"[{timestamp}] HELD SHORT SL @ ${held['sl_price']:.4f} | P&L: ${pnl:+.2f}")
                        self.held_positions.remove(held)
            ...
            if self.long_position:
                ...
                if high >= self.long_position["tp_price"]:
                    ...
                    if is_boosted and self.boost_mode_active and self.boosted_side == "LONG":
                        # Boosted TP: Close half, keep half, re-enter normal
                        pnl = self.close_position(self.long_position, self.long_position["tp_price"], "TP_BOOST_HALF", timestamp, partial=0.5)
                        self.boost_profits += pnl
                        print(f"[{timestamp}] BOOST LONG TP HALF @ ${self.long_position['tp_price']:.4f} | P&L: ${pnl:+.2f}")

                        # Keep half as held position
                        held_pos = self.long_position.copy()
                        held_pos["quantity"] = self.long_position["quantity"] * 0.5
                        held_pos["margin"] = self.long_position["margin"] * 0.5
                        self.held_positions.append(held_pos)

                        # Re-enter with normal size (still boosted mode active)
                        self.long_position = self.open_position("LONG", close, double_size=True)
                    else:
                        ...
                        # Re-enter (double if boost mode and this side is boosted)
                        double = self.boost_mode_active and self.boosted_side == "LONG"
                        self.long_position = self.open_position("LONG", close, double_size=double)
                ...
                # Check DCA
                    ...
                    # Check if DCA 2 triggered - activate boost mode
                    if new_level == 2 and not self.boost_mode_active:
                        self.activate_boost_mode("LONG", timestamp)
                        # Double the SHORT position immediately
                        if self.short_position:
                            old_qty = self.short_position["quantity"]
                            self.short_position["quantity"] *= 2
                            self.short_position["margin"] *= 2
                            self.short_position["is_boosted"] = True
                            print(f"[{timestamp}] >>> SHORT position DOUBLED: {old_qty:.4f} -> {self.short_position['quantity']:.4f}")
            ...
            if self.short_position:
                ...
                if low <= self.short_position["tp_price"]:
                    ...
                    if is_boosted and self.boost_mode_active and self.boosted_side == "SHORT":
                        # Boosted TP: Close half, keep half, re-enter normal
                        pnl = self.close_position(self.short_position, self.short_position["tp_price"], "TP_BOOST_HALF", timestamp, partial=0.5)
                        self.boost_profits += pnl
                        print(f"[{timestamp}] BOOST SHORT TP HALF @ ${self.short_position['tp_price']:.4f} | P&L: ${pnl:+.2f}")

                        # Keep half as held position
                        held_pos = self.short_position.copy()
                        held_pos["quantity"] = self.short_position["quantity"] * 0.5
                        held_pos["margin"] = self.short_position["margin"] * 0.5
                        self.held_positions.append(held_pos)

                        # Re-enter with double size (boost mode still active)
                        self.short_position = self.open_position("SHORT", close, double_size=True)
                    else:
                        ...
                        # Re-enter (double if boost mode and this side is boosted)
                        double = self.boost_mode_active and self.boosted_side == "SHORT"
                        self.short_position = self.open_position("SHORT", close, double_size=double)
                ...
                # Check DCA
                    ...
                    if new_level == 2 and not self.boost_mode_active:
                        ...
                        # Double the LONG position immediately
                        if self.long_position:
                            old_qty = self.long_position["quantity"]
                            self.long_position["quantity"] *= 2
                            self.long_position["margin"] *= 2
                            self.long_position["is_boosted"] = True
                            print(f"[{timestamp}] >>> LONG position DOUBLED: {old_qty:.4f} -> {self.long_position['quantity']:.4f}")
        ...
        for held in self.held_positions:
            if held["side"] == "LONG":
                price_change = (final_price - held["entry_price"]) / held["entry_price"]
                unrealized_held += held["margin"] * price_change * self.leverage
            else:
                price_change = (held["entry_price"] - final_price) / held["entry_price"]
                unrealized_held += held["margin"] * price_change * self.leverage

        total_unrealized = unrealized_long + unrealized_short + unrealized_held
    ...
    def print_results(self, df: pd.DataFrame, unrealized_pnl: float):
        ...
        print("\n" + "="*70)
        print("BACKTEST RESULTS - BOOST MODE")
        print("="*70)
        ...
        print(f"  Boost Profits:        ${self.boost_profits:+.2f}")
        print(f"  Held Position P&L:    ${self.held_position_profits:+.2f}")
        print(f"  Active Held Positions: {len(self.held_positions)}")
...
def main():
    print("="*70)
    print("HEDGE + DCA + BOOST MODE STRATEGY BACKTEST")
    print("="*70)

    symbol = "DOTUSDT"

    backtester = BoostModeBacktester(symbol, start_balance=100.0)
