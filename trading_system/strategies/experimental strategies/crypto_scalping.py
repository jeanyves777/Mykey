...
# Supported crypto symbols on Alpaca - V17: ALL 9 symbols for data collection
ALPACA_CRYPTO_SYMBOLS = [
    "BTC/USD", "ETH/USD", "SOL/USD", "DOGE/USD", "AVAX/USD",
    "LINK/USD", "DOT/USD", "SHIB/USD", "LTC/USD"
]

# V17: All per-symbol risk parameters REMOVED - now respects config file only
# Config file: ~/.thevolumeai/crypto_paper_trading_config.json
# V17 profitable settings: TP=1.2%, SL=1.0%, min_entry_score=5
...
class CryptoScalpingConfig(StrategyConfig):
    ...
    # Profit/Loss targets - V17: PROFITABLE on 43 real trades backtest
    # V17 Results: +$32 P&L, 74.4% win rate - TP covers 0.5% fees + profit
    target_profit_pct: float = 1.2      # V17: Net +0.7% after 0.5% fees
    stop_loss_pct: float = 1.0          # V17: 1.2:1 TP:SL ratio
    trailing_stop_pct: float = 0.0      # V17: Disabled - use fixed TP/SL only
    trailing_stop_activation: float = 999.0  # V17: Never activate
    use_trailing_stop: bool = False     # V17: Disabled
    ...
    # Entry quality score - V17: Lowered for more trades & data collection
    min_entry_score: int = 5            # V17: Lowered from 8 - collect more trade data

    # Minimum holding time (minutes) before technical exits
    min_hold_minutes: int = 3           # V11: Reduced from 15 - allow quick SL if clearly wrong
    min_profit_for_technical_exit: float = 0.3  # V11: Lower - lock in smaller profits
...
class SymbolState:
    ...
    def __init__(self, symbol: str, config: CryptoScalpingConfig):
        ...
        # V17: Use config values ONLY - no hardcoded per-symbol overrides
