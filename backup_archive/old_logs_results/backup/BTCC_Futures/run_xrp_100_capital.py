#!/usr/bin/env python3
"""
$100 Capital XRPUSDT 20x Leverage - 7 Day Backtest
===================================================
Testing with small capital to see realistic results
"""
...
print("=" * 100)
print("$100 CAPITAL - XRPUSDT @ 20x LEVERAGE - 7 DAY BACKTEST")
print("=" * 100)
print()
print("Config: TP 1.0% | SL 10% | Leverage 20x | Position Size 10% ($10)")
print("=" * 100)
print(flush=True)
...
TP_PCT = 1.0 / 100
SL_PCT = 10.0 / 100
POS_SIZE = 0.10  # 10% of account = $10 per trade
MAINT_MARGIN = 0.005

# Account - START WITH $100
balance = 100.0
initial = balance
max_balance = balance
max_dd = 0
...
print("-" * 100)
print(f"{'Bar':>6} {'Action':<8} {'Dir':<6} {'Entry':>10} {'Exit':>10} {'PnL':>10} {'Balance':>12} {'Result':<12}")
print("-" * 100)
...
for i in range(100, min(duration, len(df))):
    ...
    if position:
        ...
        if direction == 1 and low[i] <= liq_price:
            pnl = -margin
            balance += pnl
            trades.append({'bar': i, 'direction': dir_str, 'entry': entry, 'exit': liq_price, 'pnl': pnl, 'result': 'LIQUIDATION'})
            print(f"{i:>6} {'CLOSE':<8} {dir_str:<6} {entry:>10.4f} {liq_price:>10.4f} ${pnl:>+9.2f} ${balance:>11.2f} {'LIQUIDATION':<12}")
            position = None
            continue
        ...
        if direction == 1:
            if high[i] >= tp_price:
                ...
                balance += pnl
                trades.append({'bar': i, 'direction': dir_str, 'entry': entry, 'exit': tp_price, 'pnl': pnl, 'result': 'TP'})
                print(f"{i:>6} {'CLOSE':<8} {dir_str:<6} {entry:>10.4f} {tp_price:>10.4f} ${pnl:>+9.2f} ${balance:>11.2f} {'TP WIN':<12}")
                position = None
            elif low[i] <= sl_price:
                pnl = ((sl_price - entry) / entry) * margin * LEVERAGE - margin * LEVERAGE * TAKER_FEE * 2
                balance += pnl
                trades.append({'bar': i, 'direction': dir_str, 'entry': entry, 'exit': sl_price, 'pnl': pnl, 'result': 'SL'})
                print(f"{i:>6} {'CLOSE':<8} {dir_str:<6} {entry:>10.4f} {sl_price:>10.4f} ${pnl:>+9.2f} ${balance:>11.2f} {'SL LOSS':<12}")
                position = None
    ...
    if position is None:
        ...
        if signal != 0:
            ...
            print(f"{i:>6} {'OPEN':<8} {dir_str:<6} {entry:>10.4f} {'---':>10} {'---':>10} ${balance:>11.2f} TP:{tp_price:.4f}")
...
if position:
    ...
    balance += pnl
    trades.append({'bar': len(df)-1, 'direction': dir_str, 'entry': entry, 'exit': exit_price, 'pnl': pnl, 'result': 'END'})
    print(f"{len(df)-1:>6} {'CLOSE':<8} {dir_str:<6} {entry:>10.4f} {exit_price:>10.4f} ${pnl:>+9.2f} ${balance:>11.2f} {'END':<12}")
...
print()
print("=" * 100)
print("FINAL RESULTS - $100 CAPITAL @ 20x XRPUSDT")
print("=" * 100)
