"""
OPTIMIZED Multi-Symbol Forex PAPER TRADING V2 - 86%+ WIN RATE STRATEGY
Trades 10 OPTIMIZED FOREX PAIRS with RSI_REVERSAL + Session Filters
...
V2 Optimization Results (Dec 18, 2025):
- Total: +10,289 pips (100K candle backtest)
- Overall Win Rate: 86.0%
- Walk-Forward Validated: 9/10 pairs
- TP/SL: 8/20 pips (1:2.5 R:R)
- Best Session: ASIAN for most pairs
"""
...
# Import config (shared with backtest)
...
# Initialize OANDA PRACTICE ACCOUNT
print("\n" + "=" * 80)
print(f"***  {config.ACCOUNT_TYPE.upper()} TRADING MODE - OANDA {config.ACCOUNT_TYPE.upper()} ACCOUNT  ***")
print("=" * 80)
...
print("\n[4/4] Starting Optimized Paper Trading...")
print("=" * 80)
...
try:
    ...
    while True:
        ...
        if closed_trade_ids:
            ...
            for trade_id in closed_trade_ids:
                if trade_id in trade_entry_data:
                    ...
                    if oanda_trade:
                        ...
                        print(f"  {instrument} {exit_reason}: ${pnl:+.2f} ({pips:+.1f} pips) [{result_emoji}]")
                        ...
                        print(f"  {instrument}: Trade {trade_id} closed (awaiting OANDA data)")
            ...
            print(f"\n  {'=' * 65}")
            print(f"  SESSION STATS: {session_stats['wins']}W/{session_stats['losses']}L ({session_wr:.1f}%) | Total P&L: ${session_stats['total_pnl']:+.2f}")
            print(f"  {'-' * 65}")
        ...
        if len(open_trades) > 0 and iteration % 10 == 0:
            ...
            print(f"\n[{now.strftime('%H:%M:%S')}] Balance: ${balance:,.2f} | OPEN POSITIONS ({len(open_trades)}) | Open P&L: ${total_unrealized_pl:+.2f}:")
        ...
        if iteration % 12 == 0:
            ...
            if friday_no_new_trades:
                print(f"\n[{now.strftime('%H:%M:%S')}] Balance: ${balance:,.2f} | [FRIDAY] No new trades in last hour")
            else:
                print(f"\n[{now.strftime('%H:%M:%S')}] Balance: ${balance:,.2f} | Checking signals...")
            ...
            if len(open_trades) < config.MAX_CONCURRENT_POSITIONS and not friday_no_new_trades:
                for instrument in config.OPTIMIZED_PAIRS:
                    ...
                    try:
                        ...
                        print(f"\n{'=' * 80}")
                        print(f"[ENTRY SIGNAL] {signal} {instrument} ({settings['strategy']})")
    ...
    print(f"\n\n{'=' * 80}")
    print("STOPPING OPTIMIZED PAPER TRADING")
    print(f"{'=' * 80}")
        ...
        try:
            ...
            if choice == "1":
                ...
                for trade in open_trades:
                    result = client.close_trade(trade['id'])
                    if result.get('success'):
                        print(f"  {trade['instrument']} - Closed at {result['closed_price']:.5f} | P&L: ${result['pl']:+,.2f}")
    ...
    print(f"Total P&L from Closed Trades: ${session_stats['total_pnl']:+.2f}")
